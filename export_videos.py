...
Prerequisites:
    - Requires: ffmpeg with SVT-AV1 support

Usage:
    1. Edit the CONFIGURATION section below
    2. Run: ./export_videos.py
    Or: uv run export_videos.py
"""
...
def _build_audio_args(info: VideoInfo) -> list[str]:
    ...
    if AUDIO_MODE.lower() == "opus":
        ...
        # Apply channel layout filter for surround streams
        if surround_layouts:
...
def process_all(
    ...
    total = len(videos)
    console.print(f"\n[bold blue]Processing {total} video(s)[/]")
    console.print(f"  Preset {preset}, CRF {crf}")
    console.print(f"  Workers: {max_workers}\n")

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Track futures with their video info
...
def main() -> None:
    """Script entry point."""
    console.print("\n[bold]Video Converter[/] (AV1 Encoding)\n")
    ...
    try:
        ...
        if not video_paths:
            console.print(f"[yellow]No video files found in {source_dir}[/]")
            return

        console.print(f"Found {len(video_paths)} video(s) to analyze...\n")

        # Probe and filter videos
        ...
        # Process all videos
        process_all(videos_to_process, MAX_WORKERS, PRESET, CRF)

        # Summary
        console.print(
            f"\n[bold green]Done![/] {len(videos_to_process)} video(s) converted."
        )
